...
def parse_allure_tests_log(logdir):
    fields_to_extract = ['status', 'newFailed', 'description', 'time.start', 'time.duration', 'statusMessage', 'flaky',
                         'fullName', 'statusTrace']
    ...
    for fh in os.listdir(logdir):
        if fh.endswith(".json"):
            with open(os.path.join(logdir, fh)) as infile:
                ...
                out_dict = {key: ori_dict[key].split(':')[0] if isinstance(ori_dict[key], basestring) else ori_dict[key]
                            for key in fields_to_extract if ori_dict['status'] != 'passed'}
...
def load_to_es(index, mappings, data_file, file_format):
    ...
    if error:
        print(
            "Already I`ndex Mapping Exists for ES index or there is error in index mapping :{}\nInserting datafile - {} without mapping".format(
                index, data_file))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--allure_reports_path',
                        help='Absoulute Directory of Allure Reports. Ex: /var/lib/jenkins/workspace/<Jenkins Job>/')
    parser.add_argument('-e', '--es_host', help='Elasticsearch Host details, IP:PORT')
    parser.add_argument('--index_log', help='Elasticsearch Index for storing allure tests log reports',
                        default='ztap-log-1')
    parser.add_argument('--index_csv', help='Elasticsearch Index for storing allure tests csv reports',
                        default='ztap-run-1')
